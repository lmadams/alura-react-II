...
function trocaFoto(lista, fotoId, callbackAtualizaPropiedades) {
    const fotoEstadoAntigo = lista.find(foto => foto.id === fotoId);
    const novasPropriedades = callbackAtualizaPropiedades(fotoEstadoAntigo);

    const fotoEstadoNovo = Object.assign({}, fotoEstadoAntigo, novasPropriedades);
    const indexNaLista = lista.findIndex(foto => foto.id === fotoId);
    return lista.set(indexNaLista, fotoEstadoNovo);
}
